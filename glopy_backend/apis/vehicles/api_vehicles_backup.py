...
# from services.vehicle_deduplicator import VehicleDeduplicator  # DISABLED: ML server not available
...
def create_vehicle():
    ...
    if year_value and str(year_value).isdigit() and len(str(year_value)) > 4:
        print(f"⚠️ Year appears to be stock number '{year_value}', extracting from title...")
        year_value = None
    ...
    if not year_value:
        ...
        if year_match:
            ...
            print(f"🔍 Extracted year from title: {year_value}")
        else:
            year_value = 2020  # Default year if not found
            print(f"⚠️ Could not extract year from title, using default: {year_value}")
    ...
    try:
        ...
        if is_duplicate:
            print(f"DUPLICATE DETECTED: Found {len(duplicate_vehicles)} similar vehicles")
            print(f"Top match: {duplicate_vehicles[0]['vehicle']['title']} (similarity: {duplicate_vehicles[0]['similarity_score']:.2%})")
            return jsonify({
                "error": "Vehicle appears to be a duplicate",
                "duplicate_count": len(duplicate_vehicles),
                "similar_vehicles": [
                    {
                        "id": dup['vehicle']['id'],
                        "title": dup['vehicle']['title'],
                        "similarity_score": dup['similarity_score']
                    } for dup in duplicate_vehicles[:5]  # Return top 5
                ]
        ...
        if location_data:
            ...
            print("⚠️ No location data provided, location will be null")
        ...
        if make_name and make_name.isdigit():
            print(f"⚠️ Make appears to be stock number '{make_name}', extracting from title...")
            make_name = None
        ...
        if not make_name:
            ...
            for make in known_makes:
                if make.lower() in title.lower():
                    make_name = make
                    print(f"🔍 Extracted make from title: {make_name}")
                    break
        
        if not make_name:
            print("⚠️ No make provided and couldn't extract from title, using 'Unknown'")
            make_name = "Unknown"
        ...
        if model_name and model_name.isdigit():
            print(f"⚠️ Model appears to be stock number '{model_name}', extracting from title...")
            model_name = None
        ...
        if not model_name and make_name:
            ...
            if len(words) > 0:
                model_name = words[0]
                print(f"🔍 Extracted model from title: {model_name}")
        ...
        if model_name and make_id:
            ...
            print("⚠️ No model extracted, will be null")
        ...
        if raw_category and len(raw_category) > 50:
            print(f"⚠️ Category too long ({len(raw_category)} chars), truncating to 50...")
        ...
        if raw_category:
            ...
            print("⚠️ No category provided, using default 'Car'")
        ...
        def truncate_field(value, max_length, field_name=""):
            if value and len(str(value)) > max_length:
                print(f"⚠️ Truncating {field_name or 'field'} from {len(str(value))} to {max_length} chars: '{str(value)[:30]}...'")
        ...
        if "Data too long for column" in error_msg:
            ...
            if match:
                ...
                print(f"\n⚠️⚠️⚠️ FIELD CAUSING ERROR: '{field_name}' ⚠️⚠️⚠️\n")
